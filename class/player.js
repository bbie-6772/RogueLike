...
class Player {
    ...
    attack(monster, logs) {
        //랜덤 값 추출
        let playerDmg = Math.floor(Math.random() * (this.maxDmg - this.minDmg)) + this.minDmg;
        monster.hp -= playerDmg;
        if (monster.hp > 0) {
            this.totalDmg += playerDmg;
            logs.push(chalk.green(`${playerDmg} Page 만큼의 문제를 풀었습니다!`))
        } else {
            this.totalDmg += playerDmg + monster.hp;
            logs.push(chalk.green(`${playerDmg + monster.hp} Page 만큼의 문제를 풀었습니다!`))
        }
    ...
    maxHpSet(value, logs) {
        if (value > 0) {
            this.maxHp += value
            logs.push(chalk.green(`최대 정신력이 ${value} 만큼 오른 것 같습니다!`));
        } else if (value < 0) {
            this.maxHp += value
            logs.push(chalk.red(`최대 정신력이 ${Math.abs(value)} 정도 떨어졌습니다..`));
        }
    }
