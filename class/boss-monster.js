...
class bossMonster {
    ...
    heal(logs) {
        ...
        if (this.hp === this.maxHp) {
            ...
            logs.push(chalk.redBright('답안지를 잊어 버렸습니다!'));
            this.hp = this.maxHp;
            // 일반 회복
        } else {
            this.hp += monsterHeal;
            logs.push(chalk.redBright(`${monsterHeal} Page 분량의 풀이와 답들을 까먹었습니다..`));
        }
    }
    // 피로도 적용(공격)
    attack(player, logs) {
    ...
    // 과목별 특수 스킬
    skills(player, logs) {
        logs.push(chalk.whiteBright('시험지를 자세히 읽습니다!'));
        // 과목별 분리
        switch (this.type) {
            case '철학':
                logs.push(chalk.cyanBright('당신은 문제를 읽어가며 인생의 목표에 대해 생각하게되었습니다..'));
                logs.push(chalk.greenBright('시험은 왜 풀고 있는 걸까요..?'));
            ...
            case '영어':
                ...
                break;
            case '국어':
                logs.push(chalk.redBright('[보기]를 읽으며 주화 입마에 빠집니다..'));
                logs.push(chalk.redBright('정신력의 1/4이 소모됩니다!'));
                player.damaged(Math.floor(player.hp / 4), logs);
                break;
            case '체육':
                logs.push(chalk.greenBright('시험지를 읽으며 손가락 운동을 하였습니다!'));
                logs.push(chalk.greenBright(`분위기가 환기되며 최대 정신력이 ${this.hp}(Page 수)만큼 증가합니다!`));
    ...
    //집중 실패(방어)
    protect(logs) {
        logs.push(chalk.redBright('시험지가 어려워 보입니다..'));
        this.shield = true;
    }
    //Page 소모(공격받음)
    damaged(value, logs) {
        ...
        if (this.shield) {
            ...
            logs.push(chalk.greenBright(`시험지를 ${value[0]} Page 만큼 풀었습니다!`));
            //버프로 인한 추가 데미지 출력
            (value[1] > 0) ? logs.push(chalk.greenBright(`추가로 시험지를 ${value[1]} Page 만큼 풀었습니다!`)) : 0;
        }
    }
    ...
    Day(player, logs) {
        ...
        // 데미지 증가율 출력
        logs.push(chalk.redBright(`시험지를 점점 풀기 싫어집니다.. 피로도 ${mnDmg}~${mxDmg} 증가`));
