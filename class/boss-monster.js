...
class bossMonster {
    ...
    heal(logs) {
        ...
        if (this.hp === this.maxHp) {
            ...
            logs.push(
                chalk.redBright(
                    `${monsterHeal} Page 분량의 풀이와 답들을 까먹었습니다..`
                )
            );
        }
    }
    ...
    skills(player, logs) {
        ...
        switch (this.type) {
            ...
            case '국어':
                logs.push(
                    chalk.redBright('[보기]를 읽으며 주화 입마에 빠집니다..')
                );
            ...
            case '체육':
                logs.push(
                    chalk.greenBright(
                        '시험지를 읽으며 손가락 운동을 하였습니다!'
                    )
                );
                logs.push(
                    chalk.greenBright(
                        `분위기가 환기되며 최대 정신력이 ${this.hp}(Page 수)만큼 증가합니다!`
                    )
                );
    ...
    damaged(value, logs) {
        ...
        if (this.shield) {
            ...
            logs.push(
                chalk.greenBright(`시험지를 ${value[0]} Page 만큼 풀었습니다!`)
            );
            //버프로 인한 추가 데미지 출력
            value[1] > 0
                ? logs.push(
    ...
    Day(player, logs) {
        ...
        logs.push(
            chalk.redBright(
                `시험지를 점점 풀기 싫어집니다.. 피로도 ${mnDmg}~${mxDmg} 증가`
            )
        );
